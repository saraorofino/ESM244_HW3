...
ggplot(ca_counties) +
  geom_sf(fill = "cornsilk2",
          color  = "grey10",  
          size =  0.1) +
  geom_sf(data = ca_nps, fill= "darkgreen") +
...
Mean adult male lizard body mass (4.97 g ± 5.68 g, n = 57) and female lizard body mass (5.83 g ± 6.50 g, n = 75) did not differ significantly [t(`r lizard_ttest$parameter`) = `r round(lizard_ttest$statistic, 2)`, *p* = `r round(lizard_ttest$p.value, 3)`, $\alpha$ = 0.05] and the actual difference in mean weight between female and male lizards is less than one gram with a negligible effect size (Cohen's D = -0.14). 
